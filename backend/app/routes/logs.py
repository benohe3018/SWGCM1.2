...
def get_logs():
  try:
      ...
      if response.status_code != 200:
          return jsonify({'error': 'Error al obtener los logs de Heroku', 'status_code': response.status_code, 'response': response.text}), response.status_code
      ...
      if logs_response.status_code != 200:
          return jsonify({'error': 'Error al acceder a los logs de Heroku', 'status_code': logs_response.status_code, 'response': logs_response.text}), logs_response.status_code
